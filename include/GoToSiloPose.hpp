...
class GoToSiloPose : public BT::StatefulActionNode
{
    ...
    // Subscriber callback
    void silo_subscriber_callback(const std_msgs::msg::UInt8 &msg);
    ...
    void goal_response_callback(const rclcpp_action::ClientGoalHandle<NavigateThroughPoses>::SharedPtr & goal_handle_);

    // Action client result callback
    void result_callback(const GoalHandleNav::WrappedResult &result);

    // // Action client feedback callback
    void feedback_callback(GoalHandleNav::SharedPtr, const std::shared_ptr<const NavigateThroughPoses::Feedback> feedback);
